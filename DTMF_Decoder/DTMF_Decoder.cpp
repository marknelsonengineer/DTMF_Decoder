...
int APIENTRY wWinMain(
   ...
   guUMW_ERROR_IN_THREAD = RegisterWindowMessageA( APP_NAME " 9B66F531-03F2-4FE7-9D9A-7D09122EBE88" );
   if ( guUMW_ERROR_IN_THREAD == 0 ) {
      LOG_FATAL( "Failed to register a custom message.  Exiting." );
      CoUninitialize();       // Unwind COM
      return EXIT_FAILURE;
   }
...
INT_PTR CALLBACK About( HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam ) {
   ...
   switch ( message ) {
      case WM_INITDIALOG:
         ...
         WARN_BR( "Failed to set the name and version of the app.")
